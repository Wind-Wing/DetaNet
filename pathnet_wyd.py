...
def conv_module(input_tensor, filt_num, is_active, stride, layer_name, act=tf.nn.relu):
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('biases'):
      variable_summaries(biases[0])
    with tf.name_scope('Wx_plus_b'):
      initial = tf.truncated_normal([5,5,filt_num,filt_num], stddev=0.1)
...
def conv_layer(layer_name, input_tensor, filters, size, stride, padding ,freeze=False, relu=True, stddev=0.001):
...
def pooling_layer(layer_name, input_tensor, size, stride, padding):
...
def fire_layer(layer_name, input_tensor, s1x1, e1x1, e3x3, stddev=0.01,freeze=False):
  ...
  """
  sq1x1 = conv_layer(layer_name+'/squeeze1x1', input_tensor, filters=s1x1, size=1, stride=1,
...
def res_module(layer_name, input_tensor, stddev=0.01,freeze=False):
  ...
  """
  channels = input_tensor.get_shape()[3]
  feature_map_of_firstlayer = conv_layer(layer_name+'/first_layer', input_tensor, filters=channels, size= 3, stride=1,
    padding='SAME', stddev=stddev, freeze=freeze)
  feature_map_of_secondlayer = conv_layer(layer_name+'/second_layer', feature_map_of_firstlayer, filters=channels, size=3, stride=1,
    padding='SAME', stddev=stddev, freeze=freeze)
...
def Dimensionality_reduction_module(layer_name, input_tensor, c3x3, stddev=0.01,freeze=False):
  ...
  Returns:
    res_module operation.
  """
  feature_map_of_pooling = pooling_layer(layer_name+'/pooling', input_tensor,  size= 2, stride=2, padding='VALID')
  feature_map_of_convolution = conv_layer(layer_name+'/convolution', input_tensor, filters=c3x3, size=2, stride=2,
    padding='VALID', stddev=stddev, freeze=freeze)
  return  tf.concat([feature_map_of_pooling, feature_map_of_convolution], 3, name=layer_name+'/concat_pc')

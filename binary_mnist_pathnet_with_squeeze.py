...
def train():
  ...
  i = 1
  for j in range(FLAGS.M):
    layer_modules_list[j], weights_list[i,j], biases_list[i,j] = pathnet.fire_layer(net, FLAGS.filt, FLAGS.filt, FLAGS.filt, geopath[i,j], 'layer'+str(i+1)+"_"+str(j+1))
  net=np.sum(layer_modules_list)/FLAGS.M;

  net=tf.reshape(net,[-1,24*24*40])
  i = 2
  for j in range(FLAGS.M):
...
def main(_):
  ...
  train()
  end_time = time.time()
  print ('total training time is:'+str(start_time - end_time))
